...
import { buildProgram, freelancerProfilePda, ratingAvg, lamportsToSol, type FreelancerProfileAccount, type GigOnChain, type JobOnChain,
} from "@/lib/program";
...
export default function FreelancerDashboard() {
    ...
    return (
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-5 md:px-8 py-16 md:py-20">
            ...
            <div>
                <p className="text-xs font-bold tracking-[0.2em] text-[#85DABE] uppercase mb-2">Freelancer Dashboard</p>
                <h1 className="text-3xl font-extrabold text-white tracking-tight" style={{ fontFamily: "var(--font-heading, var(--font-sans))" }}>
                My Workspace
                </h1>
            </div>
            <Link href="/account/freelancer/gigs/new" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#85DABE] text-[#030712] text-sm font-bold hover:bg-[#A8E8D0] transition-colors">
                <Plus className="h-4 w-4" /> New Gig
            </Link>
            </div>

            {error && (
            <div className="mb-6 rounded-xl border border-red-500/20 bg-red-500/[0.06] px-4 py-3 text-sm text-red-300">{error}</div>
            )}
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/account/freelancer/gigs" className="text-xs text-zinc-500 hover:text-zinc-300 transition-colors">View all →</Link>
            </div>
            {gigs === null ? (
                <div className="flex items-center gap-2 py-8 text-zinc-500 text-sm"><Loader2 className="h-4 w-4 animate-spin" /> Loading…</div>
            ) : gigs.length === 0 ? (
                ...
                <Link href="/account/freelancer/gigs/new" className="text-[#85DABE] text-sm font-semibold hover:underline">Create your first gig →</Link>
                </div>
            ) : (
                ...
                {gigs.slice(0, 5).map(({ publicKey, account }) => (
                    ...
                    <Link href={`/account/freelancer/gigs/${publicKey.toBase58()}`} className="shrink-0 text-xs text-zinc-500 hover:text-white transition-colors">Edit →</Link>
                    </div>
                ))}
                </div>
            )}
            ...
            {jobs === null ? (
                ...
                <Link href="/jobs" className="text-[#85DABE] text-sm font-semibold hover:underline">Browse open jobs →</Link>
                </div>
            ) : (
                <div className="space-y-3">
                {jobs.map(({ publicKey, account }) => {
                    ...
                    <div key={publicKey.toBase58()} className="flex items-center justify-between rounded-2xl border border-white/[0.06] bg-white/[0.02] px-5 py-4 hover:border-white/[0.10] transition-colors">
                        ...
                        <p className="text-xs text-zinc-500 mt-0.5">
                            ...
                            {" · "}{s === "inProgress" ? <span className="text-[#174BD4]">In Progress</span> : <span className="text-amber-400">Delivered — Awaiting Approval</span>}
                        </p>
                        </div>
                        <Link href={`/jobs/${publicKey.toBase58()}`} className={`shrink-0 text-xs font-semibold transition-colors ${s === "inProgress" ? "text-[#85DABE] hover:text-[#A8E8D0]" : "text-zinc-500 hover:text-white"}`}>
